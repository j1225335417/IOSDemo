...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    _numRequestsLoading--;
    NSLog(@"1---%ld",(unsigned long)_numRequestsLoading);
    ...
    if(self.webTitleCallback){
       self.webTitleCallback(theTitle);
    }
}
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    ...
    if (self.webLoadErrorCallback) {
        self.webLoadErrorCallback(@"");
    }
    
//    [APPHint showErrorWithStatus:@"网络错误"];
    
    
//    [NJBHint showErrorWithStatus:@"网络请求错误"];
