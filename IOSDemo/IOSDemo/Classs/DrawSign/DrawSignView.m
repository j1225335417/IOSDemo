...
- (void)createView
{
    ...
    _image = [[UIImageView alloc]initWithFrame:CGRectMake(0 , 0, SCREEN_WIDTH, self.frame.size.height - 50)];
    
    [self addSubview:_image];
    
    
    self.drawView=[[MyView alloc]initWithFrame:CGRectMake(0 , 0, SCREEN_WIDTH, self.frame.size.height - 50)];
    ...
    btn.frame = CGRectMake(0, self.frame.size.height - 50, SCREEN_HEIGHT, 50);
    [self addSubview:btn];
    ...
    [btn addTarget:self action:@selector(saveScreen) forControlEvents:UIControlEventTouchUpInside];
